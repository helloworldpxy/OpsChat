...
- **权限审批引擎**：高危工具调用内嵌审批卡（仅此一次/始终允许/拒绝），sudo 密码即焚，审批请求会话绑定
...
- **多模型档案管理**：预置17家提供商目录，一键切换激活，API密钥Fernet加密落库
- **精确token计数**：真实 DeepSeek V3 tokenizer + provider usage 优先，会话级 token 用量展示
- **思维链审计**：完整的推理过程记录和可视化追溯，支持 CSV/JSON 导出
- **全文检索**：会话与审计 FTS5 全文检索
- **B/S架构**：浏览器访问，无需安装客户端
...
│   ├── search.py                # FTS5全文检索
│   ├── api/                     # API路由
│   │   ├── chat.py              # 对话接口（含多对话管理/审批确认）
│   │   ├── tools.py             # 工具管理
│   │   ├── audit.py             # 审计日志（含导出）
│   │   ├── settings.py          # 设置 + 模型档案接口
│   │   ├── models.py            # 模型列表
│   │   ├── custom_tools.py      # 自定义工具管理
│   │   └── search.py            # 检索接口
│   ├── core/                    # 核心模块
│   │   ├── agent.py             # Agent调度器
│   │   ├── llm_client.py        # 大模型调用
│   │   ├── chain_of_thought.py  # 思维链管理
│   │   ├── token_counter.py     # DeepSeek V3 tokenizer 精确计数
│   │   ├── title.py             # 会话标题自动生成
│   │   ├── model_profiles.py    # 模型档案服务
...
│   ├── security/                # 安全模块
│   │   ├── guardrail.py         # 安全护栏
│   │   ├── permission.py        # 权限审批引擎（规则 + sudo提权）
│   │   ├── secrets.py           # 密钥加密存储（Fernet）
│   │   ├── auth.py              # 基础登录认证
...
│   ├── models/                  # 数据模型
│   ├── utils/                   # 工具函数
│   ├── vendor/                  # 第三方资源（DeepSeek tokenizer 等）
│   └── templates/               # 前端模板
├── static/                      # 静态资源
├── tests/                       # 测试代码
├── docs/                        # 文档
...
```http
GET /api/audit/logs
GET /api/audit/export?format=csv|json   # 导出
```

### 模型档案

```http
GET    /api/settings/models                    # 列表
POST   /api/settings/models                    # 新增
POST   /api/settings/models/{id}/activate      # 激活
```

### 自定义工具

```http
GET  /api/custom-tools/          # 列表
POST /api/custom-tools/          # 创建（python 类型禁用，危险命令模板被拒）
```

### 全文检索

```http
GET /api/search?q=关键词&scope=messages|audit
```
...
- **对话页面**：智能运维对话界面，支持流式输出和Markdown渲染，内置权限审批卡（高危操作内嵌确认）
- **设置页面**：模型档案管理 + 安全 + 关于
  - 预置17家主流大模型厂商（DeepSeek、MiMo、Qwen、ChatGLM、Kimi、文心、OpenAI、Anthropic 等）
  - 支持自定义API地址
  - API Key Fernet加密存储，读接口永不返回明文
  - 一键切换激活档案、测试连接
- **审计日志页面**：可视化思维链推理过程，支持全文检索与 CSV/JSON 导出
- **系统监控页面**：实时查看系统状态
...
- **第二层**：LLM输出意图校验
- **第三层**：执行沙箱/最小权限
- **权限审批引擎**：高危工具内嵌审批卡（once/always/reject），sudo 密码即焚，审批请求会话绑定
- **自定义工具安全**：python 类型禁用、危险命令模板黑名单、high/critical 风险强制审批
- **密钥安全**：模型 API 密钥 Fernet 加密落库
...
> 服务**默认仅监听 127.0.0.1**（本机访问）。如需内网/公网访问，请将 `HOST` 改为 `0.0.0.0` 并**务必开启基础认证**（`AUTH_ENABLED=true` + 强密码）及前置 HTTPS/TLS，切勿在未认证状态下直接暴露公网。
