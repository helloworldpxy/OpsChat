...
def _validate_risk_level(risk_level: str, field: str = "risk_level"):
    """校验风险等级枚举值，非法时返回 400（而非 500）"""
    if risk_level not in _VALID_RISK_LEVELS:
        raise HTTPException(
            status_code=400,
            detail=f"{field} 取值非法，可选值: {sorted(_VALID_RISK_LEVELS)}"
        )


def _validate_tool_name(name: str):
    """校验工具名称格式，非法时返回 400（与 risk_level 校验状态码一致）"""
    if not _NAME_PATTERN.match(name):
        raise HTTPException(
            status_code=400,
            detail="工具名称只能包含字母、数字和下划线，且必须以字母或下划线开头"
        )


def _validate_command_type(command_type: str):
    """python 类型已被安全策略禁用（exec 无法沙箱化）"""
    if command_type == "python":
        raise HTTPException(
            status_code=400,
            detail="python 类型自定义工具已被安全策略禁用（exec 无法安全沙箱化，可能被用于任意代码执行），请改用 shell 类型"
        )


def _validate_command_template(template: str, tool_name: str):
    """命令模板危险片段检查（纵深防御，非唯一防线）"""
    from ..mcp.tools.custom import _DANGEROUS_SHELL_PATTERNS
    for pattern in _DANGEROUS_SHELL_PATTERNS:
        if pattern.search(template):
            raise HTTPException(
                status_code=400,
                detail=f"工具 '{tool_name}' 的命令模板包含被禁止的危险操作（{pattern.pattern}），已拒绝创建"
            )
...
async def create_custom_tool(request: CreateToolRequest):
    ...
    try:
        db = SessionLocal()
        try:
            _validate_command_type(request.command_type)
            _validate_command_template(request.command_template, request.name)
...
async def update_custom_tool(tool_id: str, request: UpdateToolRequest):
    ...
    try:
        ...
        try:
            ...
            # 安全校验：python 类型禁用 / 危险命令模板拒绝
            new_command_type = request.command_type if request.command_type is not None else tool.command_type
            new_command_template = request.command_template if request.command_template is not None else tool.command_template
            _validate_command_type(new_command_type)
            _validate_command_template(new_command_template, tool.name)
