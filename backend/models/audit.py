...
class AuditLog(Base):
    ...
    # 阶段信息
    stage = Column(String(50), nullable=False, index=True, comment="阶段名称")
...
class Conversation(Base):
    ...
    created_at = Column(DateTime, default=func.now(), comment="创建时间")
    updated_at = Column(DateTime, default=func.now(), index=True, onupdate=func.now(), comment="更新时间")
...
class ConversationMessage(Base):
    ...
    prompt_tokens = Column(Integer, default=0, comment="本消息涉及的输入 token 数（精确计数）")
    completion_tokens = Column(Integer, default=0, comment="本消息涉及的输出 token 数（精确计数）")
